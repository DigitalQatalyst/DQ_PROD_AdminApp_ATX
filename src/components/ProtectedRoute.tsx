...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ 
  ...
  // Check if user has required segment
  if (isAuthenticated && user && requiredSegments.length > 0) {
    const hasRequiredSegment = userSegment && requiredSegments.includes(userSegment.toLowerCase());
    ...
    if (!hasRequiredSegment) {
      ...
      const requiredSegmentNames = requiredSegments.map(s => segmentDisplayNames[s] || s).join(' or ');
      const currentSegmentName = userSegment ? (segmentDisplayNames[userSegment] || userSegment) : 'Unknown';
        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-50">
          <div className="max-w-md w-full bg-white shadow-lg rounded-lg p-6">
            <div className="text-center">
              ...
              <p className="mt-2 text-sm text-gray-500">
                This page is only accessible to {requiredSegmentNames} users.
              </p>
              <p className="mt-1 text-xs text-gray-400">
                Your current account type: {currentSegmentName}
              </p>
              <div className="mt-6">
                <button
